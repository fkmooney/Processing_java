...
def draw():
    ...
    ### create inset of lines that are a diff color
    ...
    offset = [x*sin(counter) for x in offset_1]
    draw_concentric_wobble(2, 200, int(w*0.35), 0.01, offset_1, angles_1, w/2, h/2) #1*w/3 
    
    ### add a second layer of the same on top
